...
exports.book_create_post = [
    ...
    // Validate fields.
    body('title', 'Titel darf nicht leer sein.').isLength({ min: 3 }).trim(),
        ...
        if (!errors.isEmpty()) {
            ...
            book.save(function (err) {
                if (err) { return next(err); }
                   ...
                   res.redirect('/catalog/books'); // original: book.url
                });
        }
    }
];
...
exports.book_delete_post = function(req, res, next) {
	...
	async.parallel({
        ...
        if (err) { return next(err); }
        if (results.book_instance.length > 0) { // bookInstances sind vorhanden.
